...
//! @return
//!
ofMatrix4x4 ICP::calcICP() {
    cv::Mat data1(4, basePointList.size(), CV_64FC1);
    cv::Mat data2(4, modifiedPointList.size(), CV_64FC1);
    
    for (int i = 0; i < basePointList.size(); i++) {
        data1.at<double>(0, i) = basePointList[i].x;
        data1.at<double>(1, i) = basePointList[i].y;
        data1.at<double>(2, i) = basePointList[i].z;
        data1.at<double>(3, i) = 1.;
    }
    basePointList.clear();
    
    for (int i = 0; i < modifiedPointList.size(); i++) {
        data2.at<double>(0, i) = modifiedPointList[i].x;
        data2.at<double>(1, i) = modifiedPointList[i].y;
        data2.at<double>(2, i) = modifiedPointList[i].z;
        data2.at<double>(3, i) = 1.;
    }
    modifiedPointList.clear();
...
//! @param modifiedData
//!
void ICP::setPoint(ofVec3f basePoint, ofVec3f modifiedPoint) {
    ofVec3d vb;
    vb.x = (double)basePoint.x;
    vb.y = (double)basePoint.y;
    vb.z = (double)basePoint.z;
    basePointList.push_back(vb);
    
    ofVec3d vm;
    vm.x = (double)modifiedPoint.x;
    vm.y = (double)modifiedPoint.y;
    vm.z = (double)modifiedPoint.z;
    modifiedPointList.push_back(vm);
...
//! @return
//!
ofMatrix4x4 ICP::cvMat2ofMat(cv::Mat cvMat) {
    ofMatrix4x4 ret;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
//! @return
//!
cv::Mat ICP::ofMat2cvMat(ofMatrix4x4 ofMat) {
